...
license: Apache-2.0
compatibility: Requires network access and npx to run @rotifer/mcp-server@0.15.1 for Arena rankings and Gene metadata.
metadata:
  author: rotifer-protocol
  version: "2.4.3"
  command: /evolve
  mcp-package: "@rotifer/mcp-server@0.15.1"
---
...
```bash
npx @rotifer/mcp-server@0.15.1 --tools=evolve
```
...
### Runtime dependency
This Skill runs [`@rotifer/mcp-server@0.15.1`](https://www.npmjs.com/package/@rotifer/mcp-server/v/0.15.1) via `npx` at runtime. The package is **fetched from npm on first use** and cached locally. This is a standard MCP Skill pattern but means you are trusting remote code — review the source before use.
...
- **Verify**: `npm view @rotifer/mcp-server@0.15.1 dist.integrity`
...
**`ROTIFER_TELEMETRY=0` stops all three** (`false` and `off` work too; any other value is not an opt-out). Until `@rotifer/mcp-server@0.15.1` it did not stop the install counter, and this section said "logged out, nothing is reported", which was not true of an install.
...
You can check rather than take our word for it: `npx @rotifer/mcp-server@0.15.1 --tools=evolve` and ask it to list its tools.
