...
| **Sends** | Gene and Arena queries to the public Rotifer API, and one version check per day to `registry.npmjs.org`. **Every MCP tool call is also logged to Rotifer Cloud** — the tool's name, the Gene it acted on, whether it succeeded, how long it took, and your Rotifer user id if you are logged in. This happens whether or not you log in, and there is no opt-out. Argument values, file contents, environment variables and your local configuration are not sent. |
